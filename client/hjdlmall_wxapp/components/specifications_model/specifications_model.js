module.exports = {
    ...
    checkAttrNum: function() {
        ...
        for (var e in i) for (var n in r) {
            ...
            if ((h = getApp().helper.inArray(i[e][0], s)) && 0 === r[n].num) for (var d in s) s[d] !== i[e][0] && o.push(s[d]);
        }
        for (var u in a) for (var _ in a[u].attr_list) {
            var p = a[u].attr_list[_];
            p.is_attr_num = !1;
            var h = getApp().helper.inArray(p.attr_id, o);
            h && (p.is_attr_num = !0);
        }
        t.setData({
            attr_group_list: a
    ...
    guigejian: function(t) {
        ...
        for (var e in i) {
            ...
            if (n.sort().join() === r.sort().join()) return i[e].num > 0 && (i[e].num -= 1, 
