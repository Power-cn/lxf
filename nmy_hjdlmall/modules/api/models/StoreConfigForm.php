...
class StoreConfigForm extends ApiModel
{
    ...
    {
        // option中存储的配置信息
        $option = Option::getList('service,web_service,web_service_url,wxapp,quick_navigation,phone_auth,good_negotiable,quick_map', $store->id, 'admin', '');
        ...
        if ($option['good_negotiable']) {
            $option['good_negotiable'] = \Yii::$app->serializer->decode($option['good_negotiable'],true);
        }else{
